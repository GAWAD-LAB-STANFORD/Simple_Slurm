suppressPackageStartupMessages({
  library(tidyverse) # Used for general purpose data manipulation, not needed by SCOPE
  library(SCOPE)
...
args <- commandArgs(trailingOnly = TRUE) # Prefix for outputted files
project <- args[1]
kb_bin_width <- as.numeric(args[2]) # This is kilobases
hg38_mapp_gc_info_dir <- args[3] # Path to files that will speed up running but unnecessary
control_dir <- args[4]
control_bam_regex <- args[5]
control_bam_suffix <- args[6] # Suffix gets trimmed off for the plot labels
experimental_dir <- args[7]
experimental_bam_regex <- args[8]
experimental_bam_suffix <- args[9] # Suffix gets trimmed off for the plot labels
...
# Functions ------------------------------------------------------
trim_sample_names <- function(sample_vector) {
  if (str_count(sample_vector, "_")[1] == 6) {
    sample_df <- as.data.frame(sample_vector) %>%
      separate(sample_vector, sep = "_", c("project", "cell_type", "tissue_origin", "sequencing_type", "TBID", "sample", "sample_number"))
    sample_vector <- sample_df[,'sample']
  }
...
plot_and_export_CNV <- function(Yhat.input = NULL, beta.hat.input = NULL, suffix, alpha.hat.input = NULL) {
  ...
  #             beta0 = beta.hat.input, filename = sprintf("%s.fig_scope_EM_%s.pdf", project, suffix))
  if (is.null(alpha.hat.input)) {
    ...
    tryCatch({
      ...
      write.table(qc_metric_qc, sprintf("%s.scope_sample_metrics.tsv", project), sep = "\t", quote = FALSE, row.names = FALSE, col.names = TRUE)
  ...
  write.table(CNV_df, sprintf("%s.scope_CNV_%s.tsv", project, suffix), sep = "\t", quote = FALSE, row.names = FALSE, col.names = TRUE)
  ...
  cnv_plot <- ggplot(CNV_df, aes(abs_start, sample, fill=CNV)) + geom_tile() + xlim(0, chr_starts[(n_chrs+1)]) +
    labs(x = "Chromosome", y = "Sample", title = sprintf("%s CNVs - %skb bins - %s", project, kb_bin_width, suffix)) +
  ...
  pdf(sprintf("%s.fig_scope_CNV_%s.pdf", project, suffix), width = plot_width, height = plot_height)
...
if (substr(control_dir, nchar(control_dir), nchar(control_dir)) != "/") {
  control_dir <- paste0(control_dir, "/")
}
if (substr(experimental_dir, nchar(experimental_dir), nchar(experimental_dir)) != "/") {
  experimental_dir <- paste0(experimental_dir, "/")
}
control_samples <- trim_sample_names(str_remove(list.files(control_dir, control_bam_regex), control_bam_suffix))
experimental_samples <- trim_sample_names(str_remove(list.files(experimental_dir, experimental_bam_regex), experimental_bam_suffix))
samples <- c(control_samples, experimental_samples)
control_indices <- rep(1:length(control_samples))
full_sample_paths <- file.path(c(rep(control_dir, length(control_samples)), rep(experimental_dir, length(experimental_samples))),
                               c(list.files(control_dir, control_bam_regex), list.files(experimental_dir, experimental_bam_regex)))
cat(sprintf("%s project - %skb bin size\n", project, kb_bin_width))
...
bam_bed_object <- get_bam_bed_n24chr(bamdir = full_sample_paths, sampname = samples, hgref = "hg38", resolution = kb_bin_width)
ref_raw <- bam_bed_object$ref

ref_raw_mapp_gc_file <- sprintf("%s/SCOPE_ref_raw_mapp_gc_%skb.tsv", hg38_mapp_gc_info_dir, kb_bin_width)
if (file.exists(ref_raw_mapp_gc_file)) {
  ref_raw_df <- read.table(ref_raw_mapp_gc_file, sep="\t", header = TRUE)
  ref_raw <- makeGRangesFromDataFrame(ref_raw_df, keep.extra.columns=TRUE)
  rm(ref_raw_df)
} else {
