...
sum_df <- df %>%
  ...
  group_by(variant_class) %>%
  	mutate(variant_class_total = sum(N)) %>%
  ungroup() %>%
  select(variant_class, total, variant_class_total) %>%
  distinct_all() %>%
  group_by(variant_class) %>%
  summarize(variant_class_proportion = sum(variant_class_total)/total) %>%
  ungroup()

fig <- ggplot(sum_df, aes(x = sample, y = variant_class_proportion, fill = variant_class)) +
...
ggsave(sprintf("%s.fig_variant_class_stacked_barplot_innerlabels.pdf", project), fig, width = 11, height = 8)
...
ggsave(sprintf("%s.fig_variant_class_sidebyside_barplot_innerlabels.pdf", project), fig, width = 20, height = 8)